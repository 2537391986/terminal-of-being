...
### 🎨 UI 从零重设计
- **配色**：暖纸白(#f0eee4)底色 + 黑墨文字，参考 Paper-White CRT Terminal 美学
- **布局**：分栏式信息架构 — 左上 SYSTEM STATUS / 右上 SESSION INFO
- **中央之眼**：SVG 虹膜放射线装饰，覆盖半透明游戏标题「存在终端」
- **分段进度条**：20 段 LED 风格经验条，替代旧光滑渐变条
- **底部双栏**：ONTOLOGICAL METRICS（战斗指标）+ LOG OUTPUT（日志）
- **命令行界面**：CLI 风格 prompt `root@being:~$_` 带闪烁光标
- **功能按钮栏**：[F1]~[F7] 黑色模块按钮，均匀分布
- **物理外壳**：米色 CRT 显示器机身 + 亮度旋钮 + 呼吸电源 LED
- **特效**：屏幕反光层 + 细微扫描线 + 四边暗角
- **字体**：VT323 英文等宽 + Noto Serif SC 中文衬线
- **响应式**：双断点 768px/480px，移动端隐去右侧面板和日志
- **面板**：浮层 panel-frame 纸白边框，淡入动画

### 🔧 代码改动
- `index.html`：完全重写（248 行），保留全部 40+ DOM id
- `style.css`：完全重写（400+ 行），暖纸白 CRT 变量体系
- `hud.js`：新增分段条更新逻辑 + 页脚时间显示 + 移除废弃 DOM 引用

## v0.8.0-crt (2026-06-23) — CRT 琥珀荧光微机风格完全重设计

### 🎨 UI 完全重设计
- **CRT 显示器外壳**：米色塑料机身 + 散热孔装饰 + 电源 LED（呼吸灯），模拟 1980 年代物理 CRT 显示器
- **琥珀荧光配色**：全界面从绿色终端 → 暖琥珀荧光（#cc9900 / #ffcc44），16 个 CSS 变量一键切换
- **三层 CRT 效果叠加**：扫描线（2px 密度）+ 四角暗角（radial-gradient）+ 荧光微闪（8s 循环），全部 `pointer-events: none`
- **DOS 文本模式布局**：双线框分割线（═══）、功能键标签（F1-F7）、终端光标闪烁块
- **字体**：Google Fonts VT323（Latin）+ SimSun（中文），还原 80 年代点阵字符感觉
- **浮层面板**：CRT 外壳框架包裹 + `panelOpen` 淡入动画
- **Canvas 内联 CRT 扫描线**：`#combat::after` 伪元素叠加

### 🔧 配套修改
- **MP 条**：新增 `#player-mp-bar` DOM 元素 + `hud.js` 百分比更新逻辑
